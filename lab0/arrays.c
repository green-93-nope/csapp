...
int main(int argc, char* argv[]) {
  ...
  // testing.
  // Answer:The test can be passed if the argument is small
  // and the value of the corresponding will be changed.
  // But if the argument is too much, it may cause the
  // stock overflow.
  fillArray(array, 10);
  ...
  // Answer: The C will not judge it is the address of the
  // value or an arrary, it only takes its argument to be
  // an address.
  fillArray(&value, 1);
  ...
  // C++ programming.
  // Answer: It can be runned, but it cause the problem
  // of stock overflow.
  free(heap_array);
  ...
  FourInts* Four_Ints = (FourInts*) malloc(sizeof(FourInts) * 1);
  fillArray((int*) Four_Ints, 4);
  assert(Four_Ints->a == 2);
  assert(Four_Ints->b == 5);
  assert(Four_Ints->c == 8);
